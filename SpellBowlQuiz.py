...
def processString(wordString, wordList):
    subStart = 0
    sub = False
    ...
    for i in range(0, len(wordString)):
        if not (wordString[i].isdigit()) and not (wordString[i] == ".") and not sub and not (wordString[i] == " "):
            ...
            wordList.append(wordString[subStart:i-1])
            sub = False
            subStart = 0
        else:
            continue
    ...
    if sub:
        wordList.append(wordString[subStart:(len(wordString))])

#cleans up all the strings
processString(wordString100, wordList100)
processString(wordString200, wordList200)
processString(wordString300, wordList300)
processString(wordString400, wordList400)
processString(wordString500, wordList500)
processString(wordString600, wordList600)
processString(wordString700, wordList700)
processString(wordString800, wordList800)
processString(wordString900, wordList900)
processString(wordString1000, wordList1000)
processString(wordString1100, wordList1100)
processString(wordString1200, wordList1200)
processString(wordString1300, wordList1300)
processString(wordString1400, wordList1400)
processString(wordString1500, wordList1500)
processString(wordString1600, wordList1600)
processString(wordString1700, wordList1700)
processString(wordString1800, wordList1800)
processString(wordString1900, wordList1900)
processString(wordString2000, wordList2000)
processString(wordString2100, wordList2100)
processString(wordString2200, wordList2200)
...
#top frame
topFrame = Frame(root, bg="white", width=1707, height=225)
topFrame.place(x=0, y=0)

#side frame
sideFrame = Frame(root, bg="#a74c0c", width=700, height = 842)
sideFrame.place(x=1007, y=225)

#logo
Label(topFrame,image=icon, bg="white").place(x=45,y=50)
...
#list dropdown title
Label(root, text="List Selection: ", font="serif 25 bold", bg="#a74c0c", fg ="black").place(x=1125, y=300)

#list dropdown
listValues = ["All Words", "Words 1 - 100"]
for j in range(1, 22):
    listValues.append("Words " + str(j*100+1) + " - " + str((j+1)*100))
listDropdown = Combobox(root, values=listValues, font=("sans serif", 20), state = "readonly", justify = "center")
listDropdown.set("Select Option")
listDropdown.place(x=1360, y=305, width = 300)
font = Font(family = "sans serif", size = 20)
root.option_add("*TCombobox*Listbox*Font", font)
root.option_add("*TCombobox*Listbox.Justify", "center")
listDropdown.bind('<<ComboboxSelected>>', lambda _ : setList())
...
audioButton = Button(root, image = audio, bg = "#D8620F", activebackground = "#D8620F", bd = 0, command=speakWord)
